...
function normalizeReplacement(record) {
  exactRecord(record, ['previousArmSha256', 'replacement'], 'PBX arm replacement');
  const previousArmSha256 = string(record.previousArmSha256, HASH, 'previousArmSha256');
  const replacement = normalizeClaim(record.replacement);
  if (previousArmSha256 === replacement.armSha256) fail('A replacement must use a fresh arm.');
  return { previousArmSha256, replacement };
}

function sqliteSchemaContract(tableName) {
  const indexName = `${tableName}_expires_idx`;
  const activeApprovalIndexName = `${tableName}_active_approval_idx`;
  const body = `(
  ...
  return Object.freeze({
    ...
    activeApprovalIndexSql: `CREATE UNIQUE INDEX ${activeApprovalIndexName} ON ${tableName}(approval_id) WHERE state <> 'superseded'`,
    createActiveApprovalIndexSql: `CREATE UNIQUE INDEX main.${activeApprovalIndexName} ON ${tableName}(approval_id) WHERE state <> 'superseded'`,
  });
}
...
function validateSqliteSchema(db, tableName, contract) {
  ...
  const activeApprovalIndexes = schemaRows(
    db,
    'SELECT type, name, tbl_name, sql FROM main.sqlite_schema WHERE name = ?',
    [contract.activeApprovalIndexName],
    'PBX active-approval index',
  );
  if (tableRecords.length !== 1 || tableRecords[0].type !== 'table' ||
      ...
      activeApprovalIndexes.length !== 1 || activeApprovalIndexes[0].type !== 'index' ||
      activeApprovalIndexes[0].name !== contract.activeApprovalIndexName ||
      activeApprovalIndexes[0].tbl_name !== tableName ||
      activeApprovalIndexes[0].sql !== contract.activeApprovalIndexSql) {
  ...
  const expectedColumns = [
    ...
    [14, 'superseded_by_arm_sha256', 'TEXT', 0, null, 0, 0],
    [15, 'superseded_at', 'INTEGER', 0, null, 0, 0],
  ];
  ...
  const expectedIndexes = new Map([
    ...
    ['approval_id', Object.freeze({
      origin: 'c', unique: 1, partial: 1, name: contract.activeApprovalIndexName,
    })],
  ...
  for (const index of indexes) {
    ...
    if (schemaInteger(index.partial) !== (expected?.partial || 0) || keyColumns.length !== 1 ||
...
function initializeSqliteSchema(db, tableName) {
  ...
  if (existingTable.length === 0) {
    const existingIndexes = schemaRows(
      db,
      'SELECT type FROM main.sqlite_schema WHERE name IN (?, ?)',
      [contract.indexName, contract.activeApprovalIndexName],
      'PBX index namespace',
    );
    ...
    try {
      db.exec(
        ...
        `${contract.createIndexSql};\n${contract.createActiveApprovalIndexSql};\nRELEASE ${savepoint};`,
      );
    } catch {
      try {
...
class MemoryPbxApprovalAttesterStore {
  ...
  claimArm(input) {
    ...
    for (const existing of this.records.values()) {
      if (existing.armSha256 === record.armSha256 ||
          (existing.approvalId === record.approvalId && existing.status !== 'superseded') ||
  ...
  supersedeArm(input) {
    const { previousArmSha256, replacement } = normalizeReplacement(input);
    const previous = this.records.get(previousArmSha256);
    if (!previous || !['pending', 'observed'].includes(previous.status) ||
        previous.approvalId !== replacement.approvalId ||
        previous.armKeyId !== replacement.armKeyId ||
        replacement.expiresAt > previous.expiresAt ||
        previous.claimedAt > replacement.claimedAt ||
        (previous.observedAt !== undefined && previous.observedAt > replacement.claimedAt) ||
        previous.expiresAt <= replacement.claimedAt) return false;
    for (const existing of this.records.values()) {
      if (existing.armSha256 === replacement.armSha256 ||
          existing.armNonce === replacement.armNonce ||
          existing.pbxCallHandle === replacement.pbxCallHandle ||
          (existing !== previous && existing.status !== 'superseded' &&
           existing.approvalId === replacement.approvalId)) return false;
    }
    // The memory store is a fixture only. SQLite supplies the crash-atomic
    // transaction; both stores retain old replay identities as tombstones.
    Object.assign(previous, {
      status: 'superseded', supersededByArmSha256: replacement.armSha256,
      supersededAt: replacement.claimedAt,
    });
    this.records.set(replacement.armSha256, { ...replacement, status: 'pending' });
    return true;
  }
...
function createSqlitePbxApprovalAttesterStore(db, {
  ...
  const supersede = db.prepare(`
    ...
    SET state = 'superseded', superseded_by_arm_sha256 = ?, superseded_at = ?
    WHERE arm_sha256 = ? AND approval_id = ? AND arm_key_id = ? AND state IN ('pending', 'observed')
      AND claimed_at <= ? AND expires_at > ?
      AND expires_at >= ?
      AND (observed_at IS NULL OR observed_at <= ?)
  `);
  let poisoned = false;
  function standaloneWrite() {
    if (poisoned || db.inTransaction !== false) {
      throw new PbxApprovalAttesterStoreError(
        'PBX_ATTESTER_STORE_TRANSACTION_UNSAFE',
        'PBX durability operations require a usable standalone SQLite connection.',
      );
    }
  }
  ...
  return Object.freeze({
    ...
    supersedeArm(input) {
      ...
      try {
        const revoked = supersede.run(
          replacement.armSha256, replacement.claimedAt, previousArmSha256,
          replacement.approvalId, replacement.armKeyId,
          replacement.claimedAt, replacement.claimedAt, replacement.expiresAt,
          replacement.claimedAt,
        ).changes === 1;
        if (!revoked || !insertClaim(replacement)) {
          db.exec('ROLLBACK');
          return false;
        }
        db.exec('COMMIT');
