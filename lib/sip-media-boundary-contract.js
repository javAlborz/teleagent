...
const TOPOLOGY_SCHEMA_V2 = 'teleagent.receiver-safe-sip-media-topology.v2';
...
// Version two adds only the two concrete FreeSWITCH-to-voice connections used
// by the pinned runtime. V1 stays exact and never acquires these permissions.
const LISTENER_DEFINITIONS_V2 = Object.freeze([
  ...LISTENER_DEFINITIONS,
  Object.freeze({ id: 'voice-media-http', service: 'voice', protocol: 'tcp', ports: 3000 }),
  Object.freeze({ id: 'voice-reverse-esl', service: 'voice', protocol: 'tcp', ports: 3002 }),
]);
const FLOW_DEFINITIONS_V2 = Object.freeze([
  ...FLOW_DEFINITIONS,
  Object.freeze({
    id: 'freeswitch-to-voice-media-http', source: 'freeswitch', destination: 'voice',
    protocol: 'tcp', sourcePorts: Object.freeze({ start: 49152, end: 65535 }), destinationPorts: 'voice-media-http',
  }),
  Object.freeze({
    id: 'freeswitch-to-voice-reverse-esl', source: 'freeswitch', destination: 'voice',
    protocol: 'tcp', sourcePorts: Object.freeze({ start: 49152, end: 65535 }), destinationPorts: 'voice-reverse-esl',
  }),
]);
...
function validateReceiverSafeSipMediaTopology(topology) {
  ...
  if (![TOPOLOGY_SCHEMA, TOPOLOGY_SCHEMA_V2].includes(topology.schema)) fail('topology schema is unsupported');
  const listenerDefinitions = topology.schema === TOPOLOGY_SCHEMA_V2 ? LISTENER_DEFINITIONS_V2 : LISTENER_DEFINITIONS;
  const flowDefinitions = topology.schema === TOPOLOGY_SCHEMA_V2 ? FLOW_DEFINITIONS_V2 : FLOW_DEFINITIONS;
  ...
  if (!Array.isArray(topology.flows) || topology.flows.length !== flowDefinitions.length) {
    fail('topology.flows must contain the canonical directional flow set');
  }
  const flows = topology.flows.map((flow, index) => {
    const definition = flowDefinitions[index];
