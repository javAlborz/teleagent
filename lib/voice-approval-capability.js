...
function fingerprintPublicKey(publicKey) {
  ...
  return crypto.createHash('sha256').update(normalized.export({
    type: 'spki',
    format: 'der',
  })).digest('hex');
}
...
function createApprovalCapabilityVerifier({
  ...
  const verificationKeyFingerprints = new Map(
    [...verificationKeys.entries()].map(([keyId, key]) => [keyId, fingerprintPublicKey(key)])
  );
  ...
  function verify(token) {
    ...
    return Object.freeze({
      ...claims,
      key_id: keyId,
      key_fingerprint: verificationKeyFingerprints.get(keyId),
    });
  }
  ...
  return Object.freeze({
    ...
    keyFingerprint(keyId) {
      return verificationKeyFingerprints.get(String(keyId || '')) || null;
    },
