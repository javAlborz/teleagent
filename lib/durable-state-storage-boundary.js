...
// Node exposes statfs by path, not fstatfs. Linux procfs resolves this path
// through the open descriptor, so the observation stays on the pinned inode
// even if the original directory pathname is renamed or replaced.
function statfsDescriptor(descriptor) {
  if (!Number.isSafeInteger(descriptor) || descriptor < 0) {
    throw new TypeError('A valid open filesystem descriptor is required.');
  }
  return fs.statfsSync(`/proc/self/fd/${descriptor}`, { bigint: true });
}
