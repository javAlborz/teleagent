...
const START_FIFO = '/run/teleagent-media/voice-start.fifo';
const REQUIRED_SOURCES = Object.freeze([
  ...
  'voice-app/lib/isolated-control-http.js',
  'voice-app/lib/local-control-request.js',
  'voice-app/lib/voice-control-routes.js',
...
function awaitHostStart({ io = fs, filename = START_FIFO, gid = process.getgid() } = {}) {
  need(filename === START_FIFO || io !== fs);
  need(Number.isSafeInteger(gid) && gid > 0 && gid < 1000);
  for (const directory of ['/', '/run', path.dirname(filename)]) {
    const info = io.lstatSync(directory);
    need(info.isDirectory() && info.uid === 0 && info.gid === 0 && !(info.mode & 0o022));
  }
  const before = io.lstatSync(filename);
  need(before.isFIFO() && before.uid === 0 && before.gid === gid &&
    (before.mode & 0o7777) === 0o440 && before.nlink === 1);
  const fd = io.openSync(filename, fs.constants.O_RDONLY | fs.constants.O_NOFOLLOW);
  try {
    const opened = io.fstatSync(fd);
    need(opened.isFIFO() && opened.dev === before.dev && opened.ino === before.ino &&
      opened.uid === before.uid && opened.gid === before.gid && opened.mode === before.mode);
    const bytes = Buffer.alloc(66);
    let length = 0;
    while (length < bytes.length) {
      const count = io.readSync(fd, bytes, length, bytes.length - length, null);
      need(Number.isSafeInteger(count) && count >= 0 && count <= bytes.length - length);
      if (count === 0) break;
      length += count;
    }
    const after = io.fstatSync(fd);
    need(after.dev === opened.dev && after.ino === opened.ino &&
      length === 65 && bytes[64] === 10);
    const generation = bytes.subarray(0, 64).toString('ascii');
    need(/^[a-f0-9]{64}$/u.test(generation));
    return generation;
  } finally { io.closeSync(fd); }
}
...
module.exports = { ADMISSION_FILE, START_FIFO, REQUIRED_SOURCES, canonical, digest, validateRuntimeAdmission,
  awaitHostStart,
