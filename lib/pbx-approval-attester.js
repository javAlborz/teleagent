...
function validateReplacement(previous, next, claimedAt) {
  const sameRequest = [
    'v', 'purpose', 'approval_id', 'job_id', 'operation', 'request_sha256',
    'plan_sha256', 'target', 'provider', 'profile', 'prompt', 'prompt_sha256',
  ];
  if (!previous?.claims || previous.keyId !== next.keyId ||
      previous.keyFingerprint !== next.keyFingerprint ||
      sameRequest.some(field => previous.claims[field] !== next.claims[field]) ||
      previous.armSha256 === next.armSha256 ||
      previous.claims.nonce === next.claims.nonce ||
      previous.claims.pbx_call_handle === next.claims.pbx_call_handle ||
      claimedAt >= previous.claims.exp || next.claims.exp > previous.claims.exp ||
      next.claims.iat < previous.claims.iat || next.claims.nbf < previous.claims.nbf) {
    fail('PBX_ATTESTER_SUPERSESSION_BINDING',
      'A replacement must preserve the exact request and deadline with a fresh call and arm.');
  }
}
...
function createPbxApprovalAttester({
  ...
  return Object.freeze({
    async attest(armToken, supersedesArmToken = null) {
      ...
      let previousArm = null;
      if (supersedesArmToken !== null) {
        previousArm = assertSynchronous(verifyArm(supersedesArmToken), 'Previous-arm verification');
        validateReplacement(previousArm, arm, claimedAt);
      }
      ...
      let claimed;
      try {
        claimed = assertSynchronous(previousArm
          ? method(store, 'supersedeArm', 'The durable attester store')(Object.freeze({
            previousArmSha256: previousArm.armSha256, replacement: claim,
          }))
          : claimArm(claim), 'Controller-arm claim or supersession');
      } catch (error) {
        if (error instanceof PbxApprovalAttesterError) throw error;
        fail('PBX_ATTESTER_STORE_ERROR', 'The durable arm claim failed.');
      }
      if (claimed !== true) {
        fail(previousArm ? 'PBX_ATTESTER_SUPERSESSION_REFUSED' : 'PBX_ATTESTER_ARM_REPLAYED',
          'The controller arm could not be claimed or atomically replaced.');
      }

      // SQLite lock acquisition/fsync may outlive the approval window. A
      // durable claim is not permission to begin an expired (or early) call.
      const collectionAtMs = timestampMilliseconds(now);
      if (collectionAtMs < claims.nbf * 1000 || collectionAtMs >= claims.exp * 1000) {
        fail('PBX_ATTESTER_ARM_WINDOW', 'The PBX collection window is no longer open.');
      }
